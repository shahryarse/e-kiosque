...
SALT = "V#kK@xNHbn>gM9?a&W~w!Zh*dL"
PEPPER = "He)U'mZz`(%#SG;6fjqAY:>x]["
...
def reserve_ticket(event_id):
    ...
    hashed_ip = hash_identifier(request.remote_addr)
    session_id = session.get('_id', str(uuid.uuid4()))
    hashed_session = hash_identifier(session_id)
    hashed_cookie = hash_identifier(request.cookies.get('session', str(uuid.uuid4())))
    ...
    ticket = Ticket(
        ...
        username=request.form.get('username') if event.collect_username else None,
...
def wiki_logout():
    ...
    # Only clear OAuth-related session data
    for key in ['oauth_request_token', 'oauth_access_token', 'wiki_username']:
        if key in session:
            del session[key]
